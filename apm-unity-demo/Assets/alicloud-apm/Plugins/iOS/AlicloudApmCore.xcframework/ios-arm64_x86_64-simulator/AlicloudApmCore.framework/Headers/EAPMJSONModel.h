...
/*
 EAPMJSONModel是json model的基类，其存在的作用是实现model属性和json之间的映射，以及自动设置等工作，使得上层model只需要声明其属性，而无需知道这些属性如何进行设置的。

 因为json数据中有些key和objc中的一些关键字或者属性方法名存在冲突，所以引入了一个keyMapper的概念，把json的key和model的属性名建立一个映射。

 model进行初始化的时候使用了一些objc rutime的功能，比如获取当前model的所有属性信息，然后再通过这些属性信息以及KVC的方式为其设置属性值。为了性能的考虑，将这些属性信息设置为model
 class object的关联对象，下次获取的时候直接从class object的关联对象中获取。
*/

@interface EAPMJSONModel : NSObject
...
+ (EAPMJSONModelKeyMapper *)modelKeyMapper;
